...
true_labels = np.array(true_labels, dtype = np.bool8)
# print(true_labels)
pred_A_labels = np.array([True, True, False, False,
                          True, True, False, False,
                          True, False, False, False, False], dtype = np.bool8)
pred_B_labels = np.array([True, True, True, True,
                          False, True, True, False,
                          True, False, True, False, False], dtype = np.bool8)
pred_C = np.array([0.8, 0.9, 0.7, 0.6,
                   0.4, 0.8, 0.4, 0.4,
                   0.6, 0.4, 0.4, 0.4, 0.2], dtype = np.float64)
pred_C_labels = np.array(pred_C.shape, dtype = np.bool8)
...
for threshold in thresholds:
    ...
    ax.annotate('C-' + str(threshold), xy = (pred_C_fpr, pred_C_tpr), xytext = (1, 1), textcoords = 'offset points')
